...
genius_csv = pd.read_csv(sys.argv[1])
    ...
    for mod in modules:
        ...
        if type(mod[0]) is not float:
            ...
            if not exit_ticket.empty:
                ...
                info[student[0]][mod[0]]['Exit']['time'] = str(ticket_diff)
            ...
            track_names = tracks[['track_title']].drop_duplicates().values
